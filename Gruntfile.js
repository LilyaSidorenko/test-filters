	...
	grunt.initConfig({
		...
		sass: {
			build: {
				...
				files: {
					'<%= config.build %>/assets/css/styles.css': '<%= config.src %>/assets/scss/styles.scss'
				}
			}
		},
		...
		sprite:{
			all: {
				src: ['<%= config.src %>/assets/images/icons/**/*.png'],
				dest: '<%= config.src %>/assets/images/spritesheet.png',
				destCss: '<%= config.src %>/assets/scss/sprites.scss'
			}
		},
